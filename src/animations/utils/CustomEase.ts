...
let _coreInitted: number,
  ...
  _bigNum = 1e20,
  _round = (value: number) => ~~(value * 1000 + (value < 0 ? -0.5 : 0.5)) / 1000,
  _bonusValidated = 1, //<name>CustomEase</name>
  _numExp = /[-+=\.]*\d+[\.e\-\+]*\d*[e\-\+]*\d*/gi, //finds any numbers, including ones that start with += or -=, negative numbers, and ones in scientific notation like 1e-8.
  _needsParsingExp = /[cLlsSaAhHvVtTqQ]/g,
  _findMinimum = (values: Array<number>) => {
    let l = values.length,
      min = _bigNum,
      i;
    for (i = 1; i < l; i += 6) {
  ...
  _normalize = (values: Array<number>, height: number, originY: number) => {
...
export class CustomEase {
  ...
  setData(data: string, config: any): CustomEase {
    config = config || {};
    data = data || "0,0,1,1";
    let values: Array<number> = (data.match(_numExp) || []).map(Number),
      closest = 1,
      points: Array<Point> = [],
      lookup: Array<Point> = [],
      precision = config.precision || 1,
      fast: number | boolean = precision <= 1,
      l: number,
      a1,
      a2,
      i,
  ...
  static create(id: string, data: string, config?: any): (p: number) => number {
  ...
  static getSVGData(ease: any, config: any): string {
    ...
    e && (e as HTMLElement).setAttribute("d", a);
    return a;
  }
}
